...
from parser.llm_enricher import enrich_scene_with_llm
...
scenes = [enrich_scene_with_llm(scene) for scene in scenes]
    ...
    for scene in script.scenes:
        ...
        if scene.scene_tone:
            for label, score in zip(
                scene.scene_tone.get("labels", []),
                scene.scene_tone.get("scores", [])
            ):
        ...
        if scene.llm_output:
            for key, value in scene.llm_output.items():
                f.write(f"{key}:\n")
